...
local selection = {
	...
	["Slashers"] = function(w, h) -- BUG: This will work fine for under 20 slashers. Have more and we'll got a problem as it'll go out of screen. Issue: We currently have exactly 20 slashers... well...
		...
		if not selectedDocument then
			GameData.DocumentPointer = 1 -- In case the GameData.DocumentPointer managed to be invalid?!? Valid or Invalid there is only hope
		...
		if slasher or (selectedDocument.DangerLevel and selectedDocument.Class) then
			draw.SimpleText(string.upper(SlashCo.Language(SlashCo.DangerLevel[slasher and slasher.DangerLevel or selectedDocument.DangerLevel]) .. " " .. SlashCo.Language(SlashCo.SlasherClass[slasher and slasher.Class or selectedDocument.Class])), "TVCDMedium", h / 1.45, w / 1.17, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		else
...
local function BuildSlasherPages()
	for _, document in pairs(SlashCoDocumentTypes["Slasher"] or {}) do
		...
		selection["Slasher-" .. (document.Slasher or document.Name)] = function(w, h)
			local row = 1
			local rowSize = w / 32
			draw.SimpleText(SlashCo.Language("documents_slasher_entry") .. " \"" .. Name .. "\"", "TVCD", h / 75, rowSize * row, color_white, 0, TEXT_ALIGN_CENTER)
